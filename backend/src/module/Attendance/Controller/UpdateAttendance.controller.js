...
const parseTimeToDate = (timeString, dateObj) => {
  ...
  return moment(`${dateStr} ${timeString}`, ['YYYY-MM-DD HH:mm:ss', 'YYYY-MM-DD HH:mm']).toDate();
};
...
export const updateAttendance = asyncHandler(async (req, res, next) => {
  ...
  if (checkInTime && status === 'Present') {
      const defaultCheckIn = parseTimeToDate( employee.defaultCheckInTime || '09:00:00', attendance.date);
      const actualCheckIn = parseTimeToDate(checkInTime, attendance.date);
      lateDurationInHours = actualCheckIn > defaultCheckIn ? moment(actualCheckIn).diff(defaultCheckIn, 'hours', true) : 0;
    }
  if (checkOutTime && status === 'Present') {
      const defaultCheckOut = parseTimeToDate( employee.defaultCheckOutTime || '17:00:00', attendance.date);
      const actualCheckOut = parseTimeToDate(checkOutTime, attendance.date);
      overtimeDurationInHours = actualCheckOut > defaultCheckOut ? moment(actualCheckOut).diff(defaultCheckOut, 'hours', true) : 0;
    }
  
  const updatedAttendance = await attendanceModel.findByIdAndUpdate(id, {
    checkInTime,
    checkOutTime,
    status,
    lateDurationInHours,
